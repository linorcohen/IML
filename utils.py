...
class AnimationButtons():
    ...
    def slider(frame_names):
        steps = [dict(args=[[i], dict(frame={'duration': 300, 'redraw': False}, mode="immediate",
...
def decision_surface(predict, xrange, yrange, density=120, dotted=False, colorscale=custom,
                     showscale=True):
    ...
    if dotted:
        return go.Scatter(x=xx.ravel(), y=yy.ravel(), opacity=1, mode="markers",
                          marker=dict(color=pred, size=1, colorscale=colorscale,
                                      reversescale=False), hoverinfo="skip", showlegend=False)
...
def create_data_bagging_utils(d=4, number_of_members=1, n_samples=1000):
    ...
    for i in range(0, d):
        ...
        for sample_curr_i, sample_curr in enumerate(list_of_array.values()):
            ...
            if (dim_half == 0):
                ...
                built_list[sample_curr_i * 2] = {
                    samples_of_half: dots_coords[dots_coords[:, 0] <= beta],
                    x_1: sample_curr[x_1],
                    x_2: beta,
                    y_1: sample_curr[y_1],
                    y_2: sample_curr[y_2],
                ...
                built_list[sample_curr_i * 2] = {
                    samples_of_half: dots_coords[dots_coords[:, 1] <= beta],
                    x_1: sample_curr[x_1],
                    x_2: sample_curr[x_2],
                    y_1: sample_curr[y_1],
                    y_2: beta,
    ...
    tags = np.hstack(
        [np.repeat(samples_["tag"], samples_["samples_of_half"].shape[0]) for samples_ in
         built_list.values()])
    return samples, tags
