...
class LogisticModule(BaseModule):
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        dot_X_w = np.dot(X, self.weights)
        return - (1 / X.shape[0]) * np.sum(y * dot_X_w - np.log(1 + np.exp(dot_X_w)))
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        """
        y_ = y - (np.exp(self.weights @ X.T) / (1 + np.exp(self.weights @ X.T)))
        return (-1 / X.shape[0]) * (y_ @ X)
...
class RegularizedModule(BaseModule):
    ...
    def __init__(self,
        ...
        """
        super().__init__()
        self.fidelity_module_, self.regularization_module_, self.lam_ = fidelity_module, \
            regularization_module, lam
