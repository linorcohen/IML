...
class GradientDescent:
    ...
    callback_: Callable[[...], None], default=default_callback
        ...
        Callable function receives as input any argument relevant for the current GD iteration.
    ...
    def __init__(self,
        ...
        learning_rate: BaseLR, default=FixedLR(1e-3)
            Learning rate strategy for retrieving the learning rate at each iteration t of the
            algorithm

        tol: float, default=1e-5
            The stopping criterion. Training stops when the Euclidean norm of w^(t)-w^(t-1) is less
        ...
        callback: Callable[[...], None], default=default_callback
            A callable function to be called after each update of the model while fitting to given
            data.
            Callable function receives as input any argument relevant for the current GD iteration.
            Arguments are specified in the `GradientDescent.fit` function
        """
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        following named arguments:
            ...
            - grad:  ndarray of shape specified by module's compute_jacobian function
                Module's jacobian with respect to the weights and at current point, over given data
                X,y
            - t: int
        ...
        """
        total_weights, best_weight, best_output = f.weights, f.weights, None
        t = 0
        delta = np.inf

        while t < self.max_iter_ and delta > self.tol_:
            w = f.weights
            f.weights = f.weights - self.learning_rate_.lr_step(t=t) * f.compute_jacobian(X=X, y=y)
            ...
            t += 1
            delta = np.sqrt(np.sum(np.power(f.weights - w, 2)))

            self.callback_(model=f, weights=f.weights, val=output, grad=jacobian, t=t, delta=delta)
