...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        Notes
        -----
        Fits model using specified `self.optimizer_` passed when instantiating class and includes
        an intercept if specified by `self.include_intercept_
        """
        if self.include_intercept_:
            X = np.c_[np.ones(len(X)), X]

        weights_samples = np.random.normal(0, 1, X.shape[1]) / np.sqrt(X.shape[1])
        model = None
        if self.penalty_ != "none":
            if self.penalty_ == "l1":
                model = L1(weights_samples)
            elif self.penalty_ == "l2":
                model = L2(weights_samples)

            base_model = RegularizedModule(
                fidelity_module=LogisticModule(weights_samples), regularization_module=model,
                lam=self.lam_, weights=weights_samples, include_intercept=self.include_intercept_)
        else:
            self.lam_ = 0
            base_model = LogisticModule(weights_samples)

        self.coefs_ = self.solver_.fit(base_model, X, y)
    ...
    def _predict(self, X: np.ndarray) -> np.ndarray:
        ...
        return np.where(predict_proba >= self.alpha_, 1, 0)
